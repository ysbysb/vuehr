      ...
      <div style="text-align: left">
        <el-dialog
          ...
          <el-row>
            <el-col :span="12">
              <div>
                <el-form-item label="模板类别:" prop="tempTypeName">
                  <el-input prefix-icon="el-icon-edit" v-model="emp.tempTypeName" size="mini" style="width: 150px"
                            placeholder="请输入模板类别"></el-input>
                </el-form-item>
              </div>
            </el-col>
            <el-col :span="12">
  ...
  export default {
    data() {
      return {
        ...
        rules: {
          tempTypeName: [{required: true, message: '必填:模板类别', trigger: 'blur'}]
    ...
    methods: {
      ...
      deleteEmp(row) {
        console.log(row)
        this.$confirm('此操作将永久删除[' + row.tempTypeName + '], 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.doDelete(row.tempTypeId);
        }).catch(() => {
        });
      },
      //删除的方法以及接口
      doDelete(ids) {
        ...
        var _this = this;
        this.delete2Request("/templateType/del/",{tempTypeId:ids}).then(resp => {
          console.log(resp)
      ...
      loadEmps() {
       console.log(this.emp)
        var _this = this;
        this.tableLoading = true;
        this.post2Request("/templateType/getPage",{pageNum:_this.currentPage,pageSize:10}).then(resp => {
      ...
      addEmp(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if (this.emp.tempTypeId) {
              alert(this.emp.tempTypeId)
              //更新
              this.tableLoading = true;
              this.putRequest("/templateType/update", _this.emp).then(resp => {
                _this.tableLoading = false;
                if (resp && resp.status == 200) {
                  var data = resp.data;
              ...
              //添加
              this.tableLoading = true;
              this.post2Request("/templateType/add", _this.emp).then(resp => {
                console.log(this.emp)
                console.log(resp)
                _this.tableLoading = false;
                if (resp && resp.status == 200) {
