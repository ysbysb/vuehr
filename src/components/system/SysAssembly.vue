      ...
      <div style="text-align: left">
        <el-dialog
          ...
          <el-row>
            <el-col :span="12">
              <div>
                <el-form-item label="组件级别:" prop="asseLevelName">
                  <el-input
                    prefix-icon="el-icon-edit"
                    v-model="emp.asseLevelName"
                    size="mini"
                    style="width: 150px"
                    placeholder="请输入组件级别"></el-input>
                </el-form-item>
              </div>
            </el-col>
            <el-col :span="12">
              <div>
                <el-form-item label="说明:" prop="remark">
                  <el-input
                    ...
                    size="mini"
                    style="width: 150px"
                    placeholder="请输入组件级别说明"></el-input>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  export default {
    data() {
      return {
        ...
        rules: {  //表单规则
          asseLevelName: [{required: true, message: '必填:组件级别', trigger: 'change'}], //组件级别验证
          remark: [{required: true, message: '必填:组件级别说明', trigger: 'blur'}],
        }
      };
    },
    ...
    methods: {
      ...
      deleteEmp(row) {
        console.log(row);
        this.$confirm('此操作将永久删除[' + row.asseLevelName + '], 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.doDelete(row.asseLevelId);
        }).catch(() => {
        });
      },
      // 删除信息
      doDelete(ids) {
        ...
        var _this = this;
        this.getRequest("/asseLevel/delete?asseLevelId=" + ids).then(resp => {
      ...
      loadEmps() {
        ...
        // this.getRequest("/employee/basic/emp?page=" + this.currentPage + "&size=10&keywords=" + this.keywords + "&asseLevelId=" + this.emp.asseLevelId).then(resp => {
        // http://localhost:8082/asse/getPage     /employee/basic/emp
        this.post2Request("/asseLevel/getPage",{pageNum:this.currentPage,pageSize:10}).then(resp => {
          console.log(resp);
      ...
      addEmp(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if (this.emp.asseLevelId) {
              //更新
              this.tableLoading = true;
              // /employee/basic/emp
              console.log(this.emp);
              this.putRequest("/asseLevel/update", this.emp).then(resp => {
                _this.tableLoading = false;
                console.log(resp);   
                if (resp && resp.status == 200) {
                  var data = resp.data;
              ...
              this.tableLoading = true;
              // /employee/basic/emp
              this.post2Request("/asseLevel/add", this.emp).then(resp => {
                _this.tableLoading = false;
                if (resp && resp.status == 200) {
                  var data = resp.data;
                  _this.dialogVisible = false; //隐藏对话框
      ...
      // initData() {
      //   var _this = this;
      //   // /asse/findLevelAndLevel
      //   this.getRequest("/asse/findLevelAndLevel").then(resp => { // /employee/basic/basicdata
      //     if (resp && resp.status == 200) {
            // var data = resp.data;
