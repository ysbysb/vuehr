function exportVUE(the, widthvalue, heightvalue, bgcolor) {
  console.log(the);
  /*获取所有的图表组件*/
  var options = []
  var websocketdata = []
  var docnamehtml = []
  var dLi = document.querySelectorAll('.showArea .vueresizepart')
  console.log(dLi.length)
  for (var i = 0; i < dLi.length; i++) {//循环遍历所有的图表组件拆出来将名字，内容，类型存起来
    var comname = dLi[i].getAttribute("name")
    var comtype = dLi[i].getAttribute("type")
    var comleft = dLi[i].getAttribute("partleft")
    var comtop = dLi[i].getAttribute("parttop")
    var comhtml = dLi[i].innerHTML
    websocketdata.push({comname: comname, comdata: null})
    options.push({comname: comname, comhtml: comhtml, comtype: comtype, comleft: comleft, comtop: comtop})
  }
  /*父组件的文本*/
  console.log(options);
  let html = `<template>
                      <div class="resume_preview_page" style="width:` + widthvalue + `px;height:` + heightvalue + `px;background-color: ` + bgcolor + `" >
  ...
  /*将父组件的文件名和文本内容存起来*/
  docnamehtml.push({chartName:the.temid, param: html})
  /*遍历每个图表组件对象判断图表类型来给相对应的子组件内容 并将名字和子组件的内容存起来*/
  options.forEach(function (v, i) {
    if (v.comtype == "getBar") {
      var template = v.comhtml
      let html = `<template>
                      <div id="` + v.comname + `" name="` + v.comname + `" style="position:absolute;left:${v.comleft}px;top:${v.comtop}px">
                          ${template}
                      </div>
                  </template>
                  <script>
  ...
  let urll = '/VueController/insert';//请求地址
  the.$http({
    url: urll,
    method: 'post',//请求方式
    // 请求体重发送的数据
